...
moverFichasCentro(Mesa, 0, ColorFichasSeleccionadas, Tablero, MesaResultante, TableroResultante):-                         %Jugador elige coger las fichas centro, se le dan las fichas
  ...
  moverFichasCentroAux(Mesa, Centro,[], ColorFichasSeleccionadas, 1,CajaResultante),
...
moverFichasCentro(Mesa, NumFactoria, ColorFichasSeleccionadas, Tablero, MesaResultante, TableroResultante):-               %Jugador elige coger las fichas centro, se le dan las fichas
  nth1(2, Mesa, Factorias),                                                                                                %y las restantes van al centro. La factoria se vacia.
  nth1(NumFactoria, Factorias, FactoriaSeleccionada),
  moverFichasCentroAux(Mesa, FactoriaSeleccionada,[], ColorFichasSeleccionadas, 1, CentroResultante),
  nth1(3, Mesa, _, Mesa1),
...
moverFichasCentroAux(Mesa, FactoriaSeleccionada, FichasCentro, ColorSeleccionado, 5, CentroResultante):-                   %Añade las fichas de la factoria al centro
  FactoriaSeleccionada \= 0,
  nth1(3, Mesa, CentroActual),
  append(CentroActual, FichasCentro, CentroResultante), !.

moverFichasCentroAux(Mesa, FactoriaSeleccionada, FichasCentro, ColorSeleccionado, 5, CentroResultante):-                   %Añade las fichas del centro a la caja
  FactoriaSeleccionada == 0,
  CentroResultante is FichasCentro, !.

moverFichasCentroAux(Mesa, FactoriaSeleccionada, FichasCentro, ColorSeleccionado, Cont, CentroResultante):-                %Comprueba que fichas van al centro/caja y cuales
  ...
  Cont1 is Cont+1,
  moverFichasCentroAux(Mesa, FactoriaSeleccionada, FichasCentroResultante, ColorSeleccionado, Cont1, CentroResultante).

moverFichasCentroAux(Mesa, FactoriaSeleccionada, FichasCentro, ColorSeleccionado, Cont, CentroResultante):-                %Comprueba que fichas van al centro/caja y cuales
  Cont1 is Cont + 1,                                                                                                       %al tablero del jugador (al jugador)
  moverFichasCentroAux(Mesa, FactoriaSeleccionada, FichasCentro, ColorSeleccionado, Cont1, CentroResultante).
...
añadirFilaReservaAMosaico(X,Reserva,Mosaico,RestoI,NuevaReserva,NuevoMosaico,RestoF):-                                         %Comprueba la fila de reserva y la añade si es correspondiente
  ...
  nth1(Num,FilaAActualizar,_,FilaPreparada),
  nth1(Num,FilaActualizada,Num,FilaPreparada),
...
contar(_, [], 0).                                                               %Hace lo mismo que count pero no devuelve una lista
contar(X, [X | T], N) :-
  !, contar(X, T, N1),
  N is N1 + 1.
contar(X, [_| T], N) :-
